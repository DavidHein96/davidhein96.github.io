...
![Differential abundance results. Prevotellaceae is enriched ~5x in White Hispanic patients relative to White Non-Hispanic patients, consistent across both LinDA and MaAsLin2.](./abundance.png)
...
![Beta diversity ordination (PCoA). Community-level separation by ethnicity is visible across both Bray-Curtis and Aitchison distance metrics.](./beta.png)
...
p1b <- p1 +
  geom_boxplot(
    aes(x = ethnicity, y = value, color = ethnicity),
    alpha = 0.1
  ) +
  stat_compare_means(
    method      = "wilcox.test",
