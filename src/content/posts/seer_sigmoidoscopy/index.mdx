---
title: "Good Enough vs. Gold Standard: Survival Analysis with RMST in the NordICC Era"
date: "2024-03-13"
categories: [R, Survival Analysis, Oncology]
description: "Survival analysis comparing flexible sigmoidoscopy vs colonoscopy using RMST, motivated by the NordICC trial's 42% adherence finding. The choice of which estimand to report isn't neutral — it's a decision about what question actually matters."
cover: ./sv_cover.jpg
---
...
ggplot(sum_data, aes(x = age_numeric, y = proportion, fill = proptype)) +
  ...
  scale_x_continuous(
    limits = c(18, 90),
    breaks = c(20, 30, 40, 45, 50, 60, 70, 80)
  ) +
  geom_vline(xintercept = 44.5, linetype = "dashed", size = 0.4) +
  geom_vline(xintercept = 49.5, linetype = "dashed", size = 0.4) +
  labs(
    x    = "Age at Diagnosis",
    y    = "Proportion of Total Cases",
    fill = "Visualization on Sigmoidoscopy"
  ) +
  theme_test()
```
...
```r
# --- Univariate models ----------------------------------------------------
age_log <- glm(can_see_sigmoid ~ age_group_final,
               data = new_data5, family = "binomial") %>%
  tidy(conf.int = TRUE) %>%
  mutate(across(c(estimate, conf.low, conf.high), exp))

race_log <- glm(can_see_sigmoid ~ race_eth,
                data = new_data5, family = "binomial") %>%
  tidy(conf.int = TRUE) %>%
  mutate(across(c(estimate, conf.low, conf.high), exp))
...
# --- Multivariate model ---------------------------------------------------
multi_log <- glm(
    can_see_sigmoid ~ race_eth + Sex + age_group_final + new_stage + year_of_diag,
    data   = new_data5,
    family = "binomial"
  ) %>%
  tidy(conf.int = TRUE) %>%
...
![IPTW-adjusted survival curves by tumor location (sigmoid-visible vs. requires colonoscopy), stratified by age group and stage. Note the crossing curves in several subgroups, which motivated the use of RMST over a standard Cox model.](./survival_curves.png)
...
ggplot(
  ...
  scale_color_manual(
    ...
    values = c("#b2182b", "#d6604d", "#f4a582",
               "#2166ac", "#4393c3", "#92c5de")
  ) +
  scale_y_continuous(
    limits = c(0, 122),
    breaks = c(0, 24, 60, 120),
    labels = c("0", "2", "5", "10")
  ) +
  labs(x = "", y = "Years of Follow Up") +
  theme_bw() +
  theme(
...
![RMST forest plot with Bonferroni-adjusted 99.8% confidence intervals, stratified by age group, stage, and tumor location. Overlapping intervals for localized disease indicate comparable survival; divergence at regional and distant stages favors sigmoid-visible tumors.](./forest.png)
...
Lin G, **Hein D**, Liu PH, Singal AG, Sanford NN. Screening Implications for Distribution of Colorectal Cancer Subsite by Age and Role of Flexible Sigmoidoscopy. *Cancers*. 2024; 16(6):1110. [https://doi.org/10.3390/cancers16061110](https://doi.org/10.3390/cancers16061110)
